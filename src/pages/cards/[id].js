...
export default function Card() {
  ...
  const [visibleAnswers, setVisibleAnswers] = useState(Array(data.length).fill(false)); // Initialize an array to track the visibility of answers
  ...
  return (
    <div className='text-white'>
      ...
      <div className="flex mainCard w-full">
        {data.map((card, index) => (
          <article key={index} className="w-[400px] h-[300px] " data-glow>
            <div data-glow></div>
            <div onClick={() => toggleAnswerVisibility(index)} className="flex flex-col items-center justify-between gap-y-2 p-3">
              <p className="inter text-2xl">{card?.question}</p>
              {visibleAnswers[index] && <p className="lexend text-[#bec2c4] opacity-75">{card?.answer}</p>}
            </div>
          </article>
        ))}
      </div>
